export const UNIVERSITY_PROJECTS = [
  // Dev Web
  {
    title: "Maison Électra&nbsp;—&nbsp;Application Angular",
    category: "Développement Web&nbsp;&&nbsp;Mobile",
    tags: ["Angular", "TypeScript", "SCSS", "Routing", "Reactive Forms"],
    description: "Single Page Application Angular pour la marque de parfums fictive Maison Électra. Catalogue interactif, système de favoris, gestion des" +
        "avis avec formulaire réactif et prévisualisation en temps réel.",
    details: "Projet réalisé en Angular dans le cadre du module Technologies du Web en L3 Informatique à l'Université de Lorraine.\nLa marque Maison Électra est une entreprise fictive créée lors du module Introduction aux Intelligences Artificielles Génératives en L2. Ce projet en constitue l'extension web, une SPA Angular présentant trois parfums (Storm Circuit, Volt Aria, Onyx Noir) avec un ensemble de fonctionnalités de navigation, de gestion des favoris et du système d'avis.\nL'application permet de parcourir le catalogue, de consulter le détail de chaque parfum, de les marquer comme favoris et de les retrouver sur une page dédiée, de lire et naviguer entre les avis, de consulter le détail d'un avis, de recommander ou supprimer un avis, et de soumettre un nouvel avis via un formulaire réactif avec prévisualisation en temps réel.\nL'architecture suit le modèle NgModule non-standalone : tous les composants sont déclarés dans AppModule, le routing est plat avec des routes paramétrées imbriquées (/catalogue/:parfumId/avis/:avisId), et les données sont gérées en mémoire via deux services singleton (ParfumService, AvisService) injectés avec inject().\nLes concepts Angular mis en pratique sont la communication parent-enfant par @Input() (ParfumList → Parfum, AvisList → Avis), la navigation programmatique via Router, les pipes natifs (date, number, titlecase, async), un pipe personnalisé NoteEtoilesPipe convertissant une note numérique en label textuel, une directive d'attribut personnalisée MaDirective gérant le surlignage au survol via ElementRef et le hook de cycle de vie OnInit.\nLe formulaire de création d'avis est implémenté en mode réactif (FormGroup, FormControl, Validators) avec un Observable construit sur valueChanges.pipe(map(...)) et consommé via le pipe async pour une prévisualisation instantanée.\nRéalisé avec Angular 21 et TypeScript.",
    links: [
      { label: "Lire", href: "#" },
  ...
  {
    title: "Series Tracker&nbsp;—&nbsp;Application Ionic",
    category: "Développement Web&nbsp;&&nbsp;Mobile",
    tags: ["Angular", "Ionic", "Capacitor", "TypeScript", "SCSS",
      "RxJS", "Reactive Forms", "API REST"],
    description: "Application mobile hybride Angular/Ionic de suivi de films et séries télévisées. Recherche en temps réel via l'API TMDB, bibliothèque locale avec gestion du statut de visionnage et formulaire d'ajout réactif.",
    details: "Projet réalisé en Angular et Ionic dans le cadre du module Développement Mobile en L3 Informatique  à l'Université de Lorraine.\nL'application permet de gérer une bibliothèque personnelle de films et de séries, d'effectuer des recherches en temps réel via l'API TMDB, de consulter la fiche détail de chaque élément, de l'ajouter à la bibliothèque et de suivre son statut de visionnage (non vu, en cours, vu). La page d'accueil propose un carrousel des contenus non vus mêlant films et séries, ainsi qu'une recherche globale croisant les deux types de contenus simultanément.\nL'architecture repose sur des NgModules non-standalone avec chargement paresseux, une navigation par onglets (/tabs/home, /tabs/movies, /tabs/series) avec routes imbriquées pour le détail et l'ajout, et trois services injectés en singleton via inject() : MovieService et SerieService gèrent les bibliothèques en mémoire, TmdbService centralise les appels HTTP à l'API TMDB.\nLes concepts Angular et Ionic mis en pratique sont la communication parent-enfant par @Input() et @Output(), la navigation programmatique via Router avec queryParams pour le retour contextuel, les directives modernes @for/@empty pour la gestion des listes à état vide, les formulaires réactifs (FormGroup, FormControl, Validators) avec Observable construit sur valueChanges.pipe(map(...)) et consommé via le pipe async pour la prévisualisation de l'affiche en temps réel, et HttpClient pour les requêtes vers l'API TMDB avec récupération des crédits en parallèle pour chaque résultat de recherche.\nLes données sont conservées uniquement en mémoire, sans persistance locale. Le build web est synchronisé vers un projet iOS natif via Capacitor.\nRéalisé seul avec Angular 20, Ionic 8, Capacitor 8 et TypeScript 5.9.",
    links: [
      { label: "Lire", href: "#" },
