(function(){
    ...
    .intro__overlay{
      ...
      background: radial-gradient(60% 40% at 50% -10%,
                  color-mix(in oklab, var(--accent, #0ea5e9) 12%, transparent), transparent 60%),
                  var(--bg, #f6f7fb);
      overflow:hidden;
    }
    html[data-theme="dark"] .intro__overlay{
      background: radial-gradient(60% 40% at 50% -10%,
    ...
    .intro__grid{
      ...
      background-image:
        linear-gradient(transparent 95%, rgba(0,0,0,.05) 96%),
        linear-gradient(90deg, transparent 95%, rgba(0,0,0,.05) 96%);
      background-size: 100% 24px, 24px 100%;
      opacity:.12;
      mix-blend-mode: multiply;
    }
    html[data-theme="dark"] .intro__grid{
    ...
    .intro__ring{
      ...
      background: conic-gradient(from 180deg, var(--accent-2, #6366f1), transparent 40%, var(--accent, #0ea5e9));
      filter: blur(14px) saturate(1.3);
      opacity:.65;
    }
    html[data-theme="dark"] .intro__ring{
    ...
    .intro__subtitle{
      font:600 clamp(10px, 2.6vw, 16px)/1 "IBM Plex Mono", monospace; color:var(--muted,#4a5568);
      letter-spacing:.12em; text-transform:uppercase; opacity:.9; margin-top:12px;
    }
    html[data-theme="dark"] .intro__subtitle{ color: var(--muted,#9aa3b2) }
    .intro__wipe{
      position:absolute; inset:auto 0 0 0; height:48%;
      background: linear-gradient(180deg, transparent, rgba(0,0,0,.12) 45%, rgba(0,0,0,.18));
      transform: translateY(100%);
    }
    html[data-theme="dark"] .intro__wipe{
